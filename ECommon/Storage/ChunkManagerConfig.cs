...
namespace ECommon.Storage
{
    public class ChunkManagerConfig
    {
        ...
        /// <summary>
        /// Chunk文件单条数据大小，字节为单位，使用于每条数据固定大小的场景
        /// </summary>
        ...
        /// <summary>
        /// Chunk可以缓存的个数上限，超过上限则不会自动缓存新创建的Chunk文件
        /// </summary>
        public readonly int ChunkCacheMaxCount;
        /// <summary>
        /// Chunk可以缓存的个数下限；低于下限则不需要进行Chunk文件的非托管内存释放处理；高于下限，则开始进行Chunk文件的非托管内存释放处理
        /// </summary>
        public readonly int ChunkCacheMinCount;
        /// <summary>
        /// 应用启动时，需要预加载到非托管内存的Chunk文件数
        /// </summary>
        ...
        /// <summary>
        /// 表示当Chunk文件无法分配非托管内存时，使用本地的环形数组进行缓存最新的记录。此属性指定本地环形数组的大小；
        /// </summary>
        public readonly int ChunkLocalCacheSize;
        /// <summary>
        /// 表示是否需要统计Chunk的写入和读取情况
        /// </summary>
